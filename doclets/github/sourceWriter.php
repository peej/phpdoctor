...
class SourceWriter extends HTMLWriter {
    ...
    function sourceWriter(&$doclet) {
        ...
        $this->_sections[5] = array('title' => 'Files', 'url' => 'overview-files.md', 'selected' => TRUE);
        ...
        foreach ($sources as $filename => $data) {
            $url = str_replace(DIRECTORY_SEPARATOR, '/', $filename);
            echo '<li><a href="source/', $url, '.md">', $filename, '</a></li>';
        }
        echo "</ul>\n";
        ...
        $this->_write('overview-files.md', 'Overview', TRUE);
        ...
        foreach ($sources as $filename => $data) {
            ...
            $this->_write('source/' . $filename . '.md', $filename, TRUE);
        }
    }
